...
function get_thumb($img, $suffix = '_thumb') {
    if (false === strpos($img, 'http://')) {
        ...
        return str_replace('.' . $ext, $suffix . '.' . $ext, $img);
    } else {
        $preg = '';
        ...
        preg_match('/(?='.preg.')/is',$img,$v);
        if(!empty($v)){
            $img = substr($img,0,strripos($img,'_'));
        }
         if (false !== strpos($img, 'taobaocdn.com') || false !== strpos($img, 'taobao.com') || false !== strpos($img, 'alicdn.com')) {
             //淘宝图片 _s _m _b
             switch ($suffix) {
                 case '_s':
                     $img = $img . '_100x100.jpg';
                     break;
                 case '_m':
                    $img = $img . '_220x220.jpg';
                     break;
                 case '_b':
                     $img = $img . '_310x310.jpg';
                     break;
             }
         }else{
            return $img;
...
function id_num($in,$to_num = false,$pad_up = false,$passKey = null)  {
 ...
 function items_check($url='',$type=''){
   ...
   if($type=='B') {
      $preg = '/\x{5df2}\x{4e0b}\x{67b6}/us';
   }elseif ($type=='C') {
      $preg = '/\x{5546}\x{54c1}\x{5f53}\x{524d}\x{4e0d}\x{53ef}\x{8d2d}\x{4e70}/us';
   }else{
    return false;
   }
   preg_match($preg, $content,$rz_check);
   sleep(1);
   if (empty($rz_check)) {
       return 'sale';
   }else{
 ...
 /*批量下架检查*/
 function items_checks($data=array()){
    $checked = array();
    ...
    foreach($data as $k=>$v){
       ...
       preg_match($preg, $v['content'],$rz_check);
       if (empty($rz_check)) {
           $checked[$k]['pass']=1;
           $checked[$k]['isshow']=1;
           $checked[$k]['status']='underway';
       }else{
           $checked[$k]['pass']=0;
           $checked[$k]['isshow']=0;
           $checked[$k]['status']='sellout';
       }
    }
  return $checked;  
 }
